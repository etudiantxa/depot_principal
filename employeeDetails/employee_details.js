...
function findEmployeeById(employeeId) {
    ...
    function findEmployeeBySpecialisation(employeeId) {
        const foundEmployee = employees.find(employee => employee.specialisation === employeeId);
        if (foundEmployee) {
        document.getElementById('employeesDetails').innerHTML =`<p>${foundEmployee.id}: ${foundEmployee.name}: ${foundEmployee.age} - ${foundEmployee.department} - ${foundEmployee.specialisation}</p>`;
        }
        else{
          document.getElementById('employeesDetails').innerHTML = 'aucun employé n\'a été trouvé avec cet ID';
